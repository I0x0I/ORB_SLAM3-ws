...
init:
	...
	#sudo docker run --name $(CONTAINERID_ID) -it -v /dev:/dev -v `pwd`/rootfs:/root/rootfs -v /home/data:/root/rootfs/data --privileged --network host $(IMAGE_ID)
	sudo docker run --name $(CONTAINERID_ID) -it -v `pwd`/rootfs:/root/rootfs -v /home/data:/root/rootfs/data --network host $(IMAGE_ID)
